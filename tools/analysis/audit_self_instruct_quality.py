...
def build_report(
    *,
    ...
    return {
        ...
        "artifact_note": "이 입력은 generated synthetic candidate pool이며 state-transition audit decision을 채워야 한다.",
...
def render_report_md(report: Mapping[str, Any]) -> str:
    lines = [
        ...
        str(report.get("artifact_note") or ""),
        "",
        f"- 생성 시각(KST): {report.get('generated_at_kst')}",
        f"- status: `{report.get('status')}`",
        f"- training_use: `{report.get('training_use')}`",
        f"- sample_md_policy: `{report.get('sample_md_policy')}`",
