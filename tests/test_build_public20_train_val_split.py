...
class BuildPublic20TrainValSplitTests(unittest.TestCase):
    def test_build_split_has_expected_size_balance_and_no_test(self) -> None:
        ...
        self.assertEqual(16, len(split["train_rows"]))
        self.assertEqual(4, len(split["val_rows"]))
        ...
        self.assertEqual({"fail": 2, "pass": 2}, report["label_counts"]["val"])
        self.assertEqual({"fail": 8, "pass": 8}, report["label_counts"]["train"])
    ...
    def test_cli_writes_three_seed_artifacts(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for seed in (11, 29, 47):
                ...
                self.assertEqual({"input", "label", "sample_id", "split"}, set(train_rows[0]))
                self.assertEqual({"input", "label", "sample_id", "split"}, set(val_rows[0]))
                self.assertEqual({"train"}, {row["split"] for row in train_rows})
                self.assertEqual({"val"}, {row["split"] for row in val_rows})
