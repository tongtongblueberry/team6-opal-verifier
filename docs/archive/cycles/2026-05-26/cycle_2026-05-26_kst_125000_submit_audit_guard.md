...
- 기본 데이터 감사 후보는 `/workspace/sinjeongmin_opal_verifier/training_data`, 로컬 `training_data` 순서다.
- `/workspace/team6` 아래로 해석되는 input/reference path는 명시 입력이어도 실패한다.
...
[Original Text/Data] `tools/analysis/data_audit.py`의 `DEFAULT_INPUT_CANDIDATES`에 과거 `/workspace/team6/training_data`가 남아 있었고, 넓은 `data/` 후보는 reference 파일까지 primary input으로 섞을 수 있었다.
→ [Exact Interpretation] 운영자가 `--input` 없이 audit를 실행하면 우리 root가 아닌 공유/legacy workspace를 읽거나, reference 자료를 train-like audit input으로 오인할 수 있었다.
→ [Detailed Explanation/Example] 기본 후보를 supervised training root로만 제한하고, `/workspace/team6`로 resolve되는 입력은 fail-closed로 막았다.
...
- `python3 -m unittest discover -s tests -v`: 60 tests OK
