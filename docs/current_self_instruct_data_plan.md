...
- 데이터 검증 절차가 끝난 뒤에는 dataset을 반드시 `train`, `val`, `test`,
  `public20_reference`로 분리한다. `public20_reference`는 shape/profile/reference와
  held-out evaluation 용도이며 generation/training prompt 또는 supervised manifest에
  들어가면 안 된다.
...
Gate A/B/C 전에는 raw synthetic sample을 "합격 데이터"로 제시하지 않는다. 이 단계의
...
Gate A/B/C를 통과한 뒤 manifest를 만들 때 데이터 partition은 다음 이름으로 고정한다.
...
- `test`: 최종 내부 holdout 평가에 쓰는 generated test split.
- `public20_reference`: public20 input-only shape/profile/reference 및 held-out fold evaluation.

금지 사항:

- public20 row를 `train`, `val`, `test` supervised manifest에 복사하기.
- public20 label을 generation prompt, judge prompt, model training target으로 쓰기.
...
`public20_labels.local.jsonl`은 aggregate distribution 비교와 held-out fold metric 계산에만 쓴다.
...
`self_instruct_candidate_schema.py`, `__init__.py`만 둔다. → [Exact Interpretation]
active datagen은 public20 input-only seed contract와 generated candidate contract만
담당한다. → [Detailed Explanation/Example] 실제 candidate generator는 별도
`generate_self_instruct_candidates.py`로 구현하되, v4/v4.1 또는 spec/gap generator를
import하면 안 된다.
...
`docs/agent_handoff.md`다. 데이터 생성, 검증 gate, 학습, 제출, cleanup 기준을 바꾸는
작업은 이 문서 묶음을 함께 점검한다.
...
3. `tools/datagen/generate_self_instruct_candidates.py`
...
7. `tools/eval/eval_self_instruct_public_seed_folds.py`
...
8. `tests/test_self_instruct_final_response_invariant.py`
...
1. `tools/datagen/generate_self_instruct_candidates.py`를 JSON contract first로 작성한다.
2. `tools/analysis/filter_self_instruct_judge.py`를 LLM-only judge filter로 작성한다.
3. Gate B public20 dimension/schema/pass-fail distribution comparison report를 작성한다.
4. Gate A qualitative sampling state-transition audit와 200-sample audit tool을 generated accepted data에 적용한다.
5. Gate A/B/C 통과 뒤 `train`, `val`, `test`, `public20_reference` split을 물리적으로 분리한다.
