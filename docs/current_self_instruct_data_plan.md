...
1. Gate C 도구를 generated manifest 후보에 적용하고 report를 archive한다.
2. 공식 prompt/metadata 계약을 따르는 LLM API generation wrapper를 작성한다.
3. LLM-only judge filtering과 duplicate/conflict filtering report를 만든다.
4. Gate A qualitative sampling state-transition audit와 200-sample audit tool을 generated accepted data에 적용한다.
5. Gate B comparison report를 generated candidate profile에 적용한다.
...
9. 모델 후보를 prompt-only, frozen RAG, 0.9B full FT, 4B QLoRA/LoRA, RAFT-style RAG+SFT 순으로 비교한다.
