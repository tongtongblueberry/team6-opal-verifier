...
3. Gate A qualitative sampling state-transition audit와 200-sample audit tool을 작성한다.
4. Gate B public20 dimension/schema/pass-fail distribution comparison report를 작성한다.
5. 5-fold evaluation runner를 작성한다.
6. ablation manifest builder를 작성한다.
7. Gate C manifest/model input path equivalence check를 training loader/first-forward smoke로 작성한다.
8. training code는 Gate A-C가 통과한 manifest가 생긴 뒤에만 연결한다.
