...
13. `tests/test_self_instruct_final_response_invariant.py`
...
13. `tools/datagen/run_self_instruct_generation.py`로 Self-Instruct output-first generation dry-run request payload/metadata writer를 구현했다. 이 도구는 LLM/API 호출과 자체 candidate 생성을 하지 않는다.
14. `tools/analysis/filter_self_instruct_judge.py`로 LLM-only judge dry-run request payload writer와 외부 judge result parser를 구현했다.
