...
`self_instruct_candidate_schema.py`, `generate_self_instruct_candidates.py`, `__init__.py`를 둔다. → [Exact Interpretation]
active datagen은 public20 input-only seed contract, generated candidate contract,
그리고 fixture-smoke candidate generation만 담당한다. → [Detailed Explanation/Example]
`generate_self_instruct_candidates.py --fixture-smoke`는 Gate A/B 연결 smoke용이며
최종 학습 데이터가 아니다. real LLM generation을 붙일 때도 v4/v4.1 또는 spec/gap
generator를 import하면 안 된다.
...
3. `tools/datagen/generate_self_instruct_candidates.py`
   ...
   - 현재 구현된 mode는 `--fixture-smoke`뿐이다. deterministic 2-row smoke pool을 만들며
     최종 학습 데이터가 아니다.
