...
- 현재 70점 수준의 문제는 이 state variable coverage가 아직 부족하다는 뜻이다.
...
[Original Text/Data] → 서버 public evaluation에서 초기 deterministic solver는 55점, `Solver` interface 수정 및 일부 rule 보강 후 70점이었다.

[Exact Interpretation] → 55점/70점은 모델 크기 문제가 아니라 parser/state coverage 문제다.
...
- 어떤 spec chunk를 근거로 rule을 적용했는지 trace가 없다.
