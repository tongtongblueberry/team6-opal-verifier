...
- Server diagnostics for `41b4df6`:
  ...
  - Metamorphic Coverage diagnostics: `pairs=1821`, `guidance_pairs=1626`, `mean_pair_mc_size=5.67`, `zero_mc_pairs=245`, `mc_cv_by_relation=0.77`, `coverage_cv_by_relation=0.32`

이 제출은 `67cd09d`와 `c613397`의 `69.50`을 넘기지는 못했다. 의미는 명확하다. MC metric/architecture는 진단력을 높였지만, solver behavior를 바꾸는 새 discriminative rule은 아직 찾지 못했다.
